...
/**
 * Abstract base source operator that handles file-based cache loading,
 * rate-limited event emission, throughput measurement, and reporting.
 * Concrete subclasses only need to implement {@link #parseLine(String)}.
 *
 * @param <T> the event type emitted by this source
 */
public abstract class AbstractNYTSourceOperator<T> extends RichParallelSourceFunction<T> {
    protected final String filePath;
    protected final int cacheSize;
    protected final int eventsPerSec;
    protected final long durationSec;
    protected final String throughputFilePrefix;

    private int localEventsPerSec;
    private transient List<T> eventCache;
    ...
    protected AbstractNYTSourceOperator(String filePath, int cacheSize, int eventsPerSec,
    ...
    /**
     * Parse a single CSV line into an event of type T.
     * @return the parsed event, or null if the line should be skipped
     */
    ...
    private void appendTimestampToConfig(long timestamp, String action, int subtaskIdx) {
        ...
        try {
            ...
            if (parentDir != null && parentDir.exists()) {
                ...
                synchronized (AbstractNYTSourceOperator.class) {
